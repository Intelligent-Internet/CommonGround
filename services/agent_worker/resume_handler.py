...
def _is_duplicate_tool_result_call_error(exc: Exception) -> bool:
    text = str(exc or "")
    return "uniq_cards_tool_result_call" in text or "duplicate key value violates unique constraint" in text
...
class ResumeHandler:
    ...
    async def _find_existing_tool_result_card(
        self,
        *,
        project_id: str,
        tool_call_id: str,
    ) -> Optional[Card]:
        call_id = safe_str(tool_call_id)
        ...
        try:
            ...
            logger.warning("Resume fail: lookup existing tool.result failed tool_call_id=%s: %s", call_id, exc)
            return None
        tool_result_cards = [c for c in (cards or []) if getattr(c, "type", "") == "tool.result"]
        if not tool_result_cards:
            return None
        tool_result_cards.sort(
            key=lambda card: getattr(card, "created_at", datetime.max.replace(tzinfo=UTC))
        )
        return tool_result_cards[0]
    ...
    async def _fail_resume(
        ...
        *,
        ...
        if not state:
            logger.warning("Resume fail: state not found for agent=%s", item.ctx.agent_id)
            return
        ...
        result_ctx = item.ctx.with_new_step(step_id).with_tool_call(tool_call_id_value)
        result_context = ToolResultContext(
            ctx=result_ctx,
            cmd_data={"after_execution": "terminate", "tool_name": "resume_handler"},
        )
        existing_error_card = await self._find_existing_tool_result_card(
            project_id=item.ctx.project_id,
            tool_call_id=tool_call_id_value,
        )
        should_save_error_card = existing_error_card is None
        if existing_error_card is not None:
            error_card = existing_error_card
        else:
        ...
        try:
            async with state_store_transaction(self.state_store) as tx:
                ...
                if should_save_error_card:
                    try:
                        await call_with_optional_conn(self.cardbox.save_card, error_card, conn=tx.conn)
                    except Exception as exc:  # noqa: BLE001
                        if not _is_duplicate_tool_result_call_error(exc):
                            raise
                        reused = await self._find_existing_tool_result_card(
                            project_id=item.ctx.project_id,
                            tool_call_id=tool_call_id_value,
                        )
                        if reused is None:
        ...
        task_ctx = item.ctx.with_bumped_epoch(int(item.ctx.turn_epoch)).with_new_step(step_id)
    ...
    async def fail_retry_exhausted(
        self,
        item: AgentTurnWorkItem,
        *,
        retry_count: int,
        ...
        tool_call_id = safe_str(item.ctx.tool_call_id)
        message = (
            f"resume retries exhausted: retry_count={int(retry_count)} max_retries={int(max_retries)} "
    ...
    async def handle(self, item: AgentTurnWorkItem) -> None:
        ...
        def _defer_current_inbox(delay_seconds: float, reason: str) -> None:
            item.inbox_finalize_action = "defer"
            item.retry_delay_seconds = max(0.0, float(delay_seconds))
            item.retry_reason = str(reason)
        ...
        if not tool_call_id:
            logger.warning("Resume: missing tool_call_id in context; fail turn.")
            await self._fail_invalid_resume(
                item,
                reason="missing tool_call_id in context",
                tool_call_id=None,
                error_code="missing_tool_call_id",
                error_message="missing tool_call_id in context",
            )
            return
        ...
        if not state:
            logger.warning(
                "Resume: State not found for %s. Deferring inbox for retry.",
                item.ctx.agent_id,
            )
            _defer_current_inbox(2.0, "state_not_found")
            return
        ...
        if not item.ctx.matches_state(state):
            ...
            if tool_result_card_id:
                try:
                    cards = await self.cardbox.get_cards([tool_result_card_id], project_id=item.ctx.project_id)
            ...
            logger.warning(
                ...
                "state=%s tool_call_id=%s tool=%s after=%s status=%s",
        ...
        if state.status == "running":
            ...
            _defer_current_inbox(1.0, "turn_running_not_suspended")
            return
        ...
        if resume.forbidden_payload_keys:
            fields = ", ".join(resume.forbidden_payload_keys)
            logger.warning("Resume: payload contains forbidden control fields (%s); fail turn.", fields)
            await self._fail_invalid_resume(
                item,
                reason=f"resume payload contains forbidden control fields: {fields}",
                tool_call_id=safe_str(tool_call_id),
                error_code="invalid_payload",
                error_message=f"resume payload contains forbidden control fields: {fields}",
            )
            return
        ...
        load_result = await fetch_and_parse_tool_result(
            cardbox=self.cardbox,
            project_id=item.ctx.project_id,
        ...
        tool_result = load_result.card
        load_err_code = load_result.error_code
        load_err_message = load_result.error_message
        if load_err_code in {
            "invalid_card_type",
            ...
            if load_err_code == "tool_call_id_mismatch":
        ...
        if not tool_result:
            if load_err_code and load_err_code != "card_not_found":
                logger.warning(
            ...
            _defer_current_inbox(1.5, "tool_result_card_not_found")
            return
        ...
        if isinstance(tool_result_meta, dict):
            ...
            if item.ctx.trace_id and not tool_result_meta.get("trace_id"):
                logger.warning("Resume: tool.result missing trace_id (call_id=%s).", tool_call_id)
            if item.ctx.parent_step_id and not tool_result_meta.get("parent_step_id"):
        ...
        if load_result.ok and isinstance(load_result.payload, dict):
            result_payload = load_result.payload
        else:
            protocol_error = ProtocolViolationError(load_result.error_message or "tool.result payload invalid")
            logger.warning("Resume: %s (tool_result_card_id=%s)", protocol_error, tool_result_card_id)
            result_payload, _ = build_error_result_from_exception(protocol_error, source="worker")
            status = STATUS_FAILED
        ...
        if not thought_card:
            ...
            _defer_current_inbox(1.5, "agent_thought_not_found")
            return
        ...
        if is_watch_heartbeat:
            ...
            wait_ctx = item.ctx.with_tool_call(str(tool_call_id)).with_new_step(str(dispatch_step_id))
            tool_name_meta = safe_str(tool_result_meta.get("function_name")) if isinstance(tool_result_meta, dict) else None
            received = await self.state_store.mark_turn_waiting_tool_received(
                ctx=wait_ctx,
                tool_result_card_id=safe_str(tool_result.card_id),
                tool_name=tool_name_meta,
            )
            if not received:
                refreshed_wait = await self.state_store.get_turn_waiting_tool(
                    ctx=item.ctx.with_tool_call(str(tool_call_id)),
                )
                refreshed_status = safe_str((refreshed_wait or {}).get("wait_status"))
                if not refreshed_wait or refreshed_status in ("applied", "expired", "dropped"):
                    logger.info(
                        "Resume: tool_call_id=%s lost waiting-set claim before apply (status=%s); idempotent consume.",
                        tool_call_id,
                        refreshed_status or "<missing>",
                    )
                    return
            await self.step_recorder.start_step(
                item=item,
            ...
            output_box_id, applied = await ensure_output_box_and_append_tool_result(
                cardbox=self.cardbox,
                ...
                waiting_ctx=item.ctx.with_tool_call(str(tool_call_id)),
            )
            if not applied:
                logger.info(
                    "Resume: tool_call_id=%s apply already claimed by another worker; idempotent consume.",
                    tool_call_id,
                )
                return
        ...
        try:
            ...
            if (
                ...
                retry_count = 0
                output_box = await self.cardbox.get_box(output_box_id, project_id=item.ctx.project_id)
                if output_box and output_box.card_ids:
                    cards = await self.cardbox.get_cards(list(output_box.card_ids), project_id=item.ctx.project_id)
                    for c in cards:
                ...
                if retry_count < self.judge_max_retries:
                    ...
                    subject = subject_pattern(
                        project_id=item.ctx.project_id,
                        channel_id=item.ctx.channel_id,
                        category="cmd",
                        component="sys",
                        target="judge",
                        suffix="retry",
                ...
                if judge_decision and judge_decision.get("action") == "retry":
                    ...
                    output_box_id = await self.cardbox.append_to_box(
                        output_box_id, [feedback_card.card_id], project_id=item.ctx.project_id
                    )
        ...
        if aggregate_after_exec == "suspend":
            ...
            if updated:
                item.ctx = item.ctx.with_bumped_epoch(next_turn_epoch)
                await self._emit_state_event(
                    item,
                    status="running",
                    output_box_id=str(output_box_id) if output_box_id is not None else None,
                ...
                step_count = await self.step_store.count_react_steps(
                    project_id=item.ctx.project_id, agent_turn_id=agent_turn_id
                )
                next_item = AgentTurnWorkItem(
                    ctx=item.ctx.with_bumped_epoch(next_turn_epoch),
                ...
                _defer_current_inbox(1.0, "resume_cas_failed_running")
                return
            return
            ...
            try:
                deliverable_ctx = item.ctx.with_bumped_epoch(int(state.turn_epoch)).with_new_step(step_id)
                output_box_id, deliverable_card_id, created_card_id = await ensure_terminated_deliverable(
                    cardbox=self.cardbox,
                    ctx=deliverable_ctx.with_tool_call(safe_str(tool_call_id)),
                    output_box_id=output_box_id,
            ...
            if not updated:
                ...
                _defer_current_inbox(1.0, "resume_cas_failed_terminate")
                return
            ...
            task_ctx = item.ctx.with_bumped_epoch(int(state.turn_epoch)).with_new_step(step_id)
            await emit_agent_task(
                nats=self.nats,
                ctx=task_ctx,
                status=task_status,
