#!/usr/bin/env python3
"""
Generate a workspace-wide NATS subject inventory (static scan).

This script scans Python sources for:
  - core.subject.format_subject(...) calls (canonical subject builder)
  - publish_event / publish_core calls
  - subscribe_cmd / subscribe_core calls
  - literal strings / f-strings that look like cg.<ver>... subjects (best-effort)

It outputs a deterministic Markdown file under docs/REFERENCE/.
"""
...
def _looks_like_cg_subject(s: str) -> bool:
    ...
    # Avoid noisy partial fragments like "cg." from f-string constants.
    # Heuristic: treat it as a subject only if it clearly contains a category or wildcard tail.
...
class _Collector(ast.NodeVisitor):
    def __init__(self, *, rel_path: str) -> None:
        ...
        # A simple per-function resolver for: name = "cg...." / f"cg...."
...
def main() -> int:
    ...
    content_lines.append("# NATS Subject 清单（自动生成）")
    content_lines.append("")
    content_lines.append("> ⚠️ 本文件由脚本自动生成，请勿手改。权威语义/约束请看：`docs/Legacy/01_protocol_l0/nats_protocol.md`。")
    content_lines.append("")
    content_lines.append("生成方式：")
    content_lines.append("")
    content_lines.append("```bash")
    content_lines.append("python scripts/admin/generate_nats_subjects_doc.py")
    content_lines.append("```")
    content_lines.append("")
    content_lines.append(f"- 扫描目录：{', '.join('`'+d+'`' for d in SCAN_DIRS)}")
    content_lines.append(f"- 排除目录名：{', '.join('`'+d+'`' for d in sorted(EXCLUDE_DIR_NAMES))}")
    content_lines.append("")
    content_lines.append("## JetStream / cmd（subscribe_cmd / publish_event）")
    content_lines.append(_render_table(jet_cmd) if jet_cmd else "_（未发现）_")
    content_lines.append("")
    content_lines.append("## JetStream / evt（subscribe_cmd / publish_event）")
    content_lines.append(_render_table(jet_evt) if jet_evt else "_（未发现）_")
    content_lines.append("")
    content_lines.append("## Core NATS / str（subscribe_core / publish_core）")
    content_lines.append(_render_table(core_str) if core_str else "_（未发现）_")
    content_lines.append("")
    content_lines.append("## 其他 / 未归类（literal / format_subject / unknown）")
    content_lines.append(_render_table(other) if other else "_（未发现）_")
    content_lines.append("")
