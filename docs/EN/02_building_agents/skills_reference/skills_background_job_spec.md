...
## Goal
- Offload long-running commands from direct `skills.run_cmd`/tool timeout paths.
- Build ordered, recoverable workflow composition with shared session state.
- Keep `Task` as a primitive, and `Job` as an ordered step workflow.
...
- `session_id`: caller-visible alias only. Runtime resolves an internal owner-scoped session key from the caller identity, so the same alias is not a cross-agent sharing key.
