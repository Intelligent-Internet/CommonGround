...
- 与沙箱/技能相关配置中可见如 `/tmp/skills-local`、`/var/lib/skills-cache`、`/proc/<pid>/cmdline` 的路径约定，原生 Windows 可能需要额外适配。
...
### UI Worker（可选；仅用于 UI Action Demo）
```bash
...
当你需要在 Principal 中实现“任务拆解 → 并行执行 → 汇总回写”，请参考：`docs/CN/03_kernel_l1/batch_manager.md`。
...
- `CG_SENDER`：NATS Header 元数据。
- `CG__PROTOCOL__VERSION`：覆盖协议版本（等价于 `config.toml` 的 `[protocol].version`）。
